...
public class McWssServer(VoiceCraftClient client) : IDisposable
{
    ...
    {
        if (genericPacket.header.eventName != "PlayerTravelled") return;
        var playerTravelledEvent = JsonSerializer.Deserialize<McWssPlayerTravelledEvent>(data);
        if (playerTravelledEvent == null || playerTravelledEvent.body.player.name != _localPlayerName) return;
