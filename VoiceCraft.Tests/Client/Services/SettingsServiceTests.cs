...
public class SettingsServiceTests
{
    ...
    [Fact]
    public async Task ResetToDefaultsAsync_ReplacesSettingsAndPersistsThem()
    {
        InitializeLocalizer();
        var storage = new FakeStorageService();
        var service = new SettingsService(storage);
        var oldUserGuid = service.UserGuid;
        service.InputSettings.InputDevice = "Mic-1";
        service.NetworkSettings.McWssHostPort = 9000;

        await service.ResetToDefaultsAsync();

        Assert.Equal("Default", service.InputSettings.InputDevice);
        Assert.Equal((ushort)8080, service.NetworkSettings.McWssHostPort);
        Assert.NotEqual(oldUserGuid, service.UserGuid);
        var saved = JsonSerializer.Deserialize(storage.StoredBytes,
            SettingsStructureGenerationContext.Default.SettingsStructure);
        Assert.NotNull(saved);
        Assert.Equal("Default", saved.InputSettings.InputDevice);
        Assert.Equal(service.UserGuid, saved.UserGuid);
    }
