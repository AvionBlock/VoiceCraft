...
public class SampleProcessingTests
{
    ...
    [Fact]
    public void SampleMixer_Read_AddsEverySampleAcrossVectorBoundary()
    {
        var source = Enumerable.Repeat(0.25f, Vector<float>.Count + 3).ToArray();
        var destination = Enumerable.Repeat(0.5f, source.Length).ToArray();

        Assert.Equal(source.Length, SampleMixer.Read(source, destination));
        Assert.All(destination, sample => Assert.Equal(0.75f, sample));
    }

    [Fact]
    public void HardClipper_Read_ClampsVectorAndScalarSamples()
    {
        var samples = Enumerable.Repeat(2f, Vector<float>.Count + 3).ToArray();
        samples[0] = -2f;
        samples[^1] = 0.25f;

        Assert.Equal(samples.Length, new SampleHardAudioClipper().Read(samples));
        Assert.Equal(-1f, samples[0]);
        Assert.All(samples[1..^1], sample => Assert.Equal(1f, sample));
        Assert.Equal(0.25f, samples[^1]);
    }

    [Fact]
    public void SampleBufferProvider_OverflowRejectsWholeWrite()
    {
        var provider = new SampleBufferProvider<int>(3);
        provider.Write([1, 2]);

        provider.Write([3, 4]);

        Span<int> result = stackalloc int[3];
        Assert.Equal(2, provider.Read(result));
