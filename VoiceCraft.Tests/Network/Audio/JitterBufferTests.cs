...
public class JitterBufferTests
{
    ...
    [Fact]
    public void Get_ZeroDropoutTime_SkipsMissingPacket()
    {
        var buffer = new JitterBuffer(TimeSpan.Zero);
        buffer.Add(new JitterPacket(10, [10]));
        Assert.True(buffer.Get(out _));
        ...
        Assert.True(buffer.Get(out var packet));
        Assert.Equal((ushort)12, packet.SequenceId);
    }

    [Fact]
    public void Reset_AcceptsSequenceThatWasPreviouslyTooOld()
    {
        var buffer = new JitterBuffer(TimeSpan.Zero);
        buffer.Add(new JitterPacket(100, [100]));
        Assert.True(buffer.Get(out _));
        buffer.Add(new JitterPacket(99, [99]));
        Assert.False(buffer.Get(out _));
        ...
        Assert.True(buffer.Get(out var packet));
        Assert.Equal((ushort)99, packet.SequenceId);
    }
}
