...
`VoiceCraft.Server.Linux.<Architecture>.zip`.
...
1. Go to where you have downloaded the `VoiceCraft.Server.Linux.<Architecture>.zip` file in your file manager and
   extract the file.
...
1. Once you have downloaded the addon above, extract the addon and place the `RP` folder in
   `<MCDirectory>/resource_packs/` and the `BP` folder in `<MCDirectory>/behavior_packs/`. You can rename the `RP` and
   `BP` folders to prevent any future conflicts.
